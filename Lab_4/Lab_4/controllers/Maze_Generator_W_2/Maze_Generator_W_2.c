...
void set_robot_position(char name[], float move) {
  WbNodeRef robot = wb_supervisor_node_get_from_def(name);
  if (!robot) {
  ...
  double pos_x = (x_min * SCALE) + move * width;
...
int main() {
  ...
  while (wb_robot_step(TIME_STEP) != -1) {
    ...
    if (time > 2.0) {
      ...
      set_robot_position("Maze-Crawler_1",  0.425);
      set_robot_position("Maze-Crawler_2",  0.1);
      add_goal(children);
      break;
    }
  }
